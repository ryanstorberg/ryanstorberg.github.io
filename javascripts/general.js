...
var resizeBoxes = function(box) {
	...
	var boxPaddingPercent = ((parseFloat($(box).css("padding").replace("px", "")) * 2) / containerWidth) * 100;
	var newRowCount = Math.round(containerWidth / 200);
	var newBoxDimensionPercent = (100 / newRowCount) - boxMarginPercent - boxPaddingPercent;
...
var checkPosition = function() {
  ...
  if (window.matchMedia('(min-width: 768px)').matches) {
		$(list).removeClass("small-icon")
  	$(list).addClass("social-icon");
  } else {
  	$(list).removeClass("social-icon");
